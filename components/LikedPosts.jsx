...
const LikedPosts = () => {
  ...
  const [userLikedBlogs, setuserLikedBlogs] = useState(null);
  const [userData, setUserData] = useState(null);
  const fetchUserData = async () => {
    const jsonValue = await AsyncStorage.getItem("@user_details");
    setUserData(jsonValue != null ? JSON.parse(jsonValue) : null);
  };
  const getBlogs_URL = `${process.env.EXPO_PUBLIC_BASE_URL}/blog/user/liked/${userData?.id}`;
  ...
  <FlatList
    ...
    renderItem={({ item }) => (
      <BlogCard
        ...
        time={item?.time} // Corrected to use `item?.time` instead of `userLikedBlogs?.time`
        blog={item?.content}
